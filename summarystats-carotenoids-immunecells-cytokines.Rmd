...
What about between interventions?

```{r}
intervention_gmcsf_subset <- meta_cytokines_long %>%
  filter(intervention != "Baseline") %>%
  filter(cytokines == "gm_csf") %>%
  select(patient_id, pre_post_intervention, cytokines, cyto_conc_pg_ml) %>%
  group_by(cytokines) %>%
  pivot_wider(names_from = pre_post_intervention,
              values_from = cyto_conc_pg_ml) %>%
  mutate(post_intervention_FC = post_Red/post_Yellow)

intervention_gmcsf_subset %>%
  summarize(mean_intervention_FC = mean(post_intervention_FC),
            mean_postYellow = mean(post_Yellow),
            SD_postYellow = sd(post_Yellow),
            mean_postRed = mean(post_Red),
            SD_postRed = sd(post_Red))
```
...
```{r}
intervention_il_12p70_subset <- meta_cytokines_long %>%
  filter(intervention != "Baseline") %>%
  filter(cytokines == "il_12p70") %>%
  select(patient_id, pre_post_intervention, cytokines, cyto_conc_pg_ml) %>%
  group_by(cytokines) %>%
...
red_il_5_cyto_subset %>%
  summarize(mean_red_FC = mean(red_FC),
            SD_red_FC = sd(red_FC),
            mean_postRed = mean(post_Red),
            SD_red_postRed = sd(post_Red),
            mean_preRed = mean(pre_Red),
            SD_red_preRed = sd(pre_Red))
...
```{r}
intervention_il_5_subset <- meta_cytokines_long %>%
  filter(intervention != "Baseline") %>%
  filter(cytokines == "il_5") %>%
  select(patient_id, pre_post_intervention, cytokines, cyto_conc_pg_ml) %>%
  group_by(cytokines) %>%
...
* CD14+ MDSC monocytes shows a huge mean fold change between interventions! (~ 44)
