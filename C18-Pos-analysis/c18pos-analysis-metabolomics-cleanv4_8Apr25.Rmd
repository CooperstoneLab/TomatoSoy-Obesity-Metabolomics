...
write_csv(post_for_mummichog,
          "post-mummichog-list-c18pos.csv")
...
stds_df_for_stats_wide <- df_for_stats %>%
  ...
  dplyr::select(c(1:11),
                "76.0757_0.642", #TMAO
                "156.0769_0.621", #Histidine
                "162.1126_0.642", #L-carnitine
                "166.0863_2.395", #Phenylalanine
                "180.0658_3.481", #Hippuric acid
                "205.0971_2.931", #Tryptophan
                "255.0655_5.039", #Daidzein
                "316.248_6.107" #Decanoylcarnitine
                )
...
df_for_stats_wide_noOutlier <- df_for_stats_wide %>%
  filter(Subject != 6106,
         Subject != 6112)
...
forcorr_lycsoymetabslog2_immuno_noOutlier <- df_for_stats_wide_noOutlier %>%
  mutate(il_5 = carot_immunology_meta_noOutlier$il_5,
         ...
         tnf_a = carot_immunology_meta_noOutlier$tn_fa,
         total_lyc = carot_immunology_meta_noOutlier$total_lyc) %>%
  select(il_5, il_12p70, gm_csf, tnf_a, total_lyc,
         all_of(sigmetabs_lycsoy_effect))
...
Here, I am going to look at correlations between lyc/soy assoc features and significant cytokines (+ TNF-alpha, which was not significant but trending downwards post-Red)


```{r}
# correlation table
corr_results_lycsoymetabs_immuno_noOutlier <- forcorr_lycsoymetabslog2_immuno_noOutlier %>% 
...
lycsoy_ggcorr_noOutlier <- round(cor(forcorr_lycsoymetabslog2_immuno_noOutlier, 
                              method = "spearman"), 
                          digits = 2)


(corplot_lycsoy_ggcorr_noOutlier <- lycsoy_ggcorr_noOutlier %>%
  ggcorrplot(lab = TRUE, 
             outline.color = "white",type = "full",
...
ggsave(plot = lyc_soy_immuno_metabs_ggcorrplot,
       filename = "plots and figures/prepostRedFC-c18-pos-lycsoy-Metabs-and-immunology-corrplot.svg",
       width = 15,
       height = 15)
```
