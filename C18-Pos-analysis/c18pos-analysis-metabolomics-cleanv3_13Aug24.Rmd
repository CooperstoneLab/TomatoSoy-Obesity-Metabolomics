...
```{r, fig.width=8.5, fig.height=6.5}
(PCA.colby.prevspost_outliers <- PCAtools::biplot(p_outliers,
                               lab = NULL,
...
```{r, warning=FALSE}
horn <- PCAtools::parallelPCA(log2_abundata_forPCAtools)
...
biplot(p,
       ...
       subtitle = "Log2 transformed data, C18 (+), subjects 6106 and 6112 removed, no QCs \n99% confidence level ellipses")
...
### Loadings 
```{r, fig.width=10}
plotLoadings(multilevelPCA.result, 
             ndisplay = 15,
...
(plot_multilevelPCA_manual <- multilevelPCA_points %>% 
  ggplot(aes(x = PC1, y = PC2, 
    ...
    scale_color_manual(values = "black") +  
    theme_minimal() +
    labs(x = "PC1, 21% variation",
         y = "PC2, 10% variation",
         fill = "Timepoint",
         title = "Multilevel Principal Components Analysis Scores Plot",
         subtitle = "Log2 transformed data, C18 (+) including outlier subjects"))
...
Here, I am comparing pre- to post-intervention for both yellow and tomato soy (Red) juice interventions individually, and post-yellow to post-red intervention. I am using the log transformed values of rel abundance since parametric tests assume normality.
...
Daidzein is the only feature that I have a standard for here that is significantly different between post-red vs. post-control.
