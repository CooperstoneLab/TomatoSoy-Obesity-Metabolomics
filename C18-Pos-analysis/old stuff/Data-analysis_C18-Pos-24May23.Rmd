...
omicsdata <- read_csv("Feature lists/C18Pos-Filtered-Data-24May23redo_3878features.csv")
...
df_combined_sep <- df_combined_sep %>%
  select(sample_ID, pre_post, Intervention, everything())
...
Update May 3, 2023: I've been seeing outliers in unsupervised analyses. So to handle this, I think it is best to filter out metabolites that are only present in one person. So I will remove metabolites that are missing from at least 44 samples 
...
## Notes 
Looks like subject 6106 is an outlier! Need to do some cross-checks
- immunology data and plasma carotenoid levels. 
* 6106 does not look odd for any plasma analysis data
- check sample prep + urine conc.
* this was a sample in which 2 of their timepoints had less urine than others, but I adjusted accordingly
- keep this in mind for the rest of the analysis modes
- will see what the data looks like without the outlier
...
imp_clust_omicsdata_forPCAtools <- imp_clust_omicsdata_forPCAtools %>%
  select(!contains("QC")) # remove QC observations
...
# remove outliers from both df
abundata_reordered_forPCAtools <- abundata_reordered_forPCAtools %>%
  select(!contains("6106")) %>%
  select(!contains("6112"))
...
# pca
p <- pca(abundata_reordered_forPCAtools, 
         metadata = metadata_forPCAtools, 
         scale = TRUE # unit variance scale
)
...
```{r, echo=FALSE}
horn <- parallelPCA(abundata_reordered_forPCAtools)
...
ggplotly(PCA.colby.prevspost,
         tooltip = "text") # trying to figure out how to make hover text to be subj
...
```{r, fig.width=12, fig.height=7.5, message=FALSE}
  ...
  eigencorplot(p,
    components = getComponents(p, 1:20), # get components that account for 80% variance
...
ctrl_t.test_paired <- df_for_stats %>%
  filter(Intervention == "Yellow") %>%
  select(Subject, pre_post, mz_rt, rel_abund_log2) %>%
  group_by(mz_rt) %>%
...
post_t.test_paired <- df_for_stats %>%
  filter(pre_post == "post") %>%
  select(Subject, Intervention, mz_rt, rel_abund_log2) %>%
  group_by(mz_rt) %>%
...
write_csv(higher_postRed,
          "intervention-comp-sig-RED-C18Pos-24May23.csv")

write_csv(higher_postYellow,
          "intervention-comp-sig-YELLOW-C18Pos-24May23.csv")
```
...
write_csv(higher_postRed_noOutlier,
          "intervention-comp-sig-RED-nooutliers-C18Pos-24May23.csv")

write_csv(higher_postYellow_noOutlier,
          "intervention-comp-sig-YELLOW-nooutliers-C18Pos-24May23.csv")
```
...
write_csv(red_higher_post,
          "pre-post-sig-RED-C18Pos-24May23.csv")
```
...
write_csv(red_higher_post_noOutlier,
          "pre-post-sig-RED-nooutliers-C18Pos-24May23.csv")
```
...
write_csv(yellow_higher_post,
          "pre-post-sig-YELLOW-C18Pos-24May23.csv")
```
...
write_csv(yel_higher_post_noOutlier,
          "pre-post-sig-YELLOW-nooutliers-C18Pos-24May23.csv")
```
