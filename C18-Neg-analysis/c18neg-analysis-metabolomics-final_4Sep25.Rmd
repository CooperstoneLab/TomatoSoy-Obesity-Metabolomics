...
ggplotly(plot_mixOmicsPCA_manual,
         tooltip = "text")
```
...
(PCAbiplot_reg <- mixOmicsPCA_points_norm %>%
  ggplot() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  ...
  geom_label_repel(data = mixOmicsPCA_loadings %>%
                    filter(PC2 %in% loadings_PC2$importance),
                   aes(x = PC1*9, y = PC2*180, label = mz_rt), 
                   size = 2.5, segment.color = "transparent",
                   direction = "both", max.overlaps = 15) +
  scale_fill_manual(values = c("lemonchiffon1",
                                 "yellow2",
...
```{r}
ggsave("plots and figures/PCAbiplot_C18NEG.svg", 
       PCAbiplot_reg, height = 5)
```
...
sigmetabs_tomato_effect %>% 
  filter(Feature_ID == "naringenin-7-glucuronide") %>%
  ...
  labs(x = "",
       ...
       title = "Naringenin-7-O-glucuronide",
       subtitle = "Confirmed with authentic standard") +
...
```{r}
write_csv(anno_imp_metabind_clust_log2_noQCs, file = "meta_omics_noQCs.csv")
```
