...
Let's look at the top 20 features that are driving separation between 6106 and the rest of the subjects.
```{r}
# plot top 20 features that have at least a 95% cos2 (correlation)
...
To try and handle outliers, I came up with filtering out metabolites that are only present in one person.
...
imp_no6106_log2 <- imp_metabind_clust_tidy_log2 %>%
  dplyr::select(!rel_abund) %>%
  filter(mz_rt != contrib_vars_6106$name) %>%
  pivot_wider(names_from = mz_rt,
              values_from = rel_abund_log2)
...
loadings_pcatools_outliers <- getLoadings(p_outliers)
```
